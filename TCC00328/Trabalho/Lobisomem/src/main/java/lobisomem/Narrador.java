...
import lobisomem.participantes.*;
...
public class Narrador {
    ...
    public void matarParticipante(Participante alvo){
        ...
        //checa se é um Caçador, e se for, mata o alvo
        if("Caçador".equals(alvo.getPapel().getId())){
            String[] strigStatus;
            Participante participanteEscolhido = null;
            for(Participante participante : participantes){
                strigStatus = mesa.getStatus(participante).split(" ");
                for(int i=0; i<strigStatus.length; i++){
                    String statusProcurado = "marcado:" + alvo.getNome();
                    if(statusProcurado.equals(strigStatus[i])) participanteEscolhido = participante;
                }
            }
            for(Participante participante : participantes){
                System.out.println(participante);
            }
            matarParticipante(participanteEscolhido);
            for(Participante participante : participantes){
                System.out.println(participante);
            }
        }
    ...
    public void rodadaNoturna(){
        ...
        for(int i = 1; i < mesa.getParticipantes().size(); i++){
            Papel papel = mesa.getParticipantes().get(i).getPapel();
            papel.acaoNoturnaIA();
        }
        operaStatus();
    ...
    public void acaoDoJogador(Usuario jogador){
        ...
        while(entradaInvalida){
            try{
                ...
                else System.out.println("Por favor, entrar com um número entre 0 e " + (ultimaOpcao + 1) + ".");
            }
    ...
    public boolean terminouJogo(){
        ...
        if(!(participantes.get(0) instanceof Usuario)){
        ...
        if(numEvil == 0){
            ...
            for(i = 0; i < mesaInicial.size(); i++){   
                System.out.println(mesaInicial.get(i).getNome() + " possuia o papel: " + mesaInicial.get(i).getPapel().getId());
            }
            return true;
        }
        return false;
