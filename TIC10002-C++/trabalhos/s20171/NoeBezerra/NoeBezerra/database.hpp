...
public:
    ...
    int size() {
        return count;
    } // Informa o número de elementos armazenados
};
...
private:
    ...
    public:
        ...
        int leftrotate(tree<T, Ref>* structure) {
            ...
            if (caller->ptrtop) {
                father = caller->ptrtop;
            } else {
                father = nullptr;
            }
            if (father) {
                if (father->ptrleft == caller) {
                    father->ptrleft = this;
                } else {
                    father->ptrright = this;
                }
            } else {
                structure->father(this);
            }
        ...
        void getdata(queue<T>* listing) {
            if (ptrleft) {
                ptrleft->getdata(listing);
            }
            T* data = new T;
            *data = *this->content;
            listing->push(data);
            if (ptrright) {
                ptrright->getdata(listing);
            }
        }
        ...
        T* search(Ref KEY) {
            if (this->key == KEY) {
                return this->content;
            } else if (ptrleft && this->key > KEY) {
                return (*this->ptrleft).search(KEY);
            } else if (ptrright && this->key < KEY) {
                return (*this->ptrright).search(KEY);
            } else {
                return nullptr;
            }
        }
        ...
        void getrange(queue<T>* list, Ref min, Ref max) {
            ...
            if (ptrright) {
                ptrright->getrange(list, min, max);
            }
        }
    };

    void father(node* change) {
        ptrfather = change;
    }
    node* ptrfather;
    int count;
public:
    ...
    queue<T> range(Ref min, Ref max) {
        queue<T> list;
        if (ptrfather) {
            ptrfather->getrange(&list, min, max);
        }
        return list;
    }
};
